...
shell-server-cannot = Сервен не может выполнить это.
...
shell-cannot-run-command-from-server = Вы не можете выполнить эту команду.
...
shell-wrong-arguments-number-need-specific = Нужно { $properAmount ->
    [one] аргумент
    [few] аргумента
    *[other] аргументов
}, было { $currentAmount ->
...
shell-argument-must-be-boolean = Аргумент должен быть логическим (true | false)
...
shell-entity-is-not-mob = Цель не является существом!
...
shell-invalid-bool = Неверный логический элемент.
shell-entity-uid-must-be-number = EntityUid должно быть числом.
...
shell-target-player-does-not-exist = Игрок не существует!
shell-target-entity-does-not-have-message = Сущность не имеет { $missing }!
